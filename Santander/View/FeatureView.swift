        ...
        ScrollView(.horizontal, showsIndicators: false){
            LazyHGrid(rows: [GridItem()]){
                ForEach(viewModel.features, id: \.id) { feature in
                    Rectangle().foregroundColor(.clear).frame(width: 110, height: 96).background(.white).cornerRadius(6).shadow(color: .black.opacity(0.15), radius: 4.5, x: .zero, y: 4)
                        .overlay {
                            VStack {
                                ...
                                Text(feature.description)
                                    ...
                                    .kerning(0.14).foregroundColor(Color(red: 0.57, green: 0.57, blue: 0.57))
                            }
